  ...
  upload = ()=>{
    ...
    setTimeout(() => {
      if(this.state.username === null && this.state.password ===null){
        ...
        var url = 'http://webhub.gtfs-ride.org:8080/checkUser/'+this.state.username+'/password/'+this.state.password+'/';
        fetch(url)
        .then(res => res.json())
        .then((mycount) => {
          if(mycount.valid === true){
            //alert("received true")
            window.open("http://webhub.gtfs-ride.org:8080/?User="+this.state.username+"#/",'_self');
          }
          else{
